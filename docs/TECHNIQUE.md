...
```bash
docker compose --profile train run --rm train python scripts/train_poppy.py \
    --config configs/poppy_robust.yaml --timesteps 2048 --n-envs 1 --seed 0 --log-dir logs/smoke
```
...
**Pendant l'entraînement** — TensorBoard, déjà lancé par `--profile train` :

```bash
docker compose --profile train up tensorboard
```

Puis <http://localhost:6006>. Courbes de récompense, longueur d'épisode, pertes.

**Après** — deux scripts, en natif :
...
requis : c'est le premier réflexe utile quand on découvre le projet.

```bash
python scripts/visualize.py models/2026-04-08_21-29-14/best_model.zip --episodes 5
```

Une politique qui se déroule, à l'écran. `--no-render` pour ne mesurer que les
récompenses, `--fps 30` pour ralentir.

**Pourquoi pas dans Docker ?** Ces deux scripts ouvrent une fenêtre OpenGL. La faire
sortir d'un conteneur sous Windows demande un serveur X et une demi-journée de
réglages, pour zéro bénéfice. Ils tournent en natif ; l'installation est décrite dans
[`DOCKER.md`](DOCKER.md) § Sans Docker.

Pour une vidéo **sans écran**, c'est `evaluate.py --video` (§5), qui rend en `osmesa`
dans le conteneur.
...
| Vidéo sans écran | `docker compose --profile eval run --rm eval python scripts/evaluate.py --model <modèle> --episodes 1 --video logs/eval.mp4` |
...
| Le robot, en natif | `python scripts/viewer.py` |
| Une politique, en natif | `python scripts/visualize.py models/<date>/best_model.zip --episodes 5` |
